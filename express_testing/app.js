...
app.get('/midpoint', (req, res) => {
  ...
  if (!lat1 || !lat2 || !lon1 || !lon2) {
    return res
      .status(400)
      .send('Please include valid coordinates for each location');
  }
...
app.get('/frequency', (req, res) => {
  ...
  // Validate response
  // Throw an error if string is undefined
  if (!str) {
    return res
      .status(400)
      .send('Invalid request')
  ...
  // Count then number of unique chars
  const unique = Object.keys(record).length;

  // Find the average frequency
  const average = str.length / unique;
  // Create values for the highest 
  let highest = '';
  let highestVal = 0;

  // Determine the char with most occurrences
  Object.keys(record).forEach(key => {
    if (record[key] > highestVal) {
      highestVal = record[key];
      highest = key;
    }
  });
